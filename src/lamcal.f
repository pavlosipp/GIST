C-----------------------------------------------------------------------
C  17.11.89        LAST MODIFICATION 03.02.95     UHS: lamcal DATA   D
C-----------------------------------------------------------------------
C
     ...
     &                  vbt,vbsq,tsc,lsx,lmnb,vlt,vlp,psivt,psivp,pgppv,
     &                  pgttv,pgtpv,al,gla,dna,dnb)
C
...
C.. Local Scalars .. 
      integer i,JK,L
      real dvol,FPPI,FTPI,RVQS,tsi,ZVS
C 
C.. External Calls .. 
      external LAMNEW
C 
...
C    $                       * TPONP
          pGTTV(JK) = 0.5 * (GTTV(JK,I)+GTTV(JK,I-1)) / vjac(jk,i)
          pGTPV(JK) = -0.5*(GTPV(JK,I)+GTPV(JK,I-1))/vjac(jk,i)
          pGPPV(JK) = 0.5 * (GPPV(JK,I)+GPPV(JK,I-1)) / vjac(jk,i)
C
        end do
...
C
C     COMPUTE B(SUB)T, B(SUB)P, J, I, ETC.
C
        CIV(I) = 0.
        CJV(I) = 0.
        VVP(I) = 0.
